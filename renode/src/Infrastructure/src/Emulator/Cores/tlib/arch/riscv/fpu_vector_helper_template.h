...
{                                                                                               \
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {                                            \
        ...
        switch (eew) {                                                                          \
        case 32:                                                                                \
            acc = glue(HELPER, _s)(env, acc, (float32)V32(vs2,ei));                          \
            break;                                                                              \
        case 64:                                                                                \
            acc = glue(HELPER, _d)(env, acc, (float64)V64(vs2,ei));                          \
            break;                                                                              \
        }                                                                                       \
    }                                                                                           \
    switch (eew) {                                                                              \
        case 32:                                                                                \
            WR_V32(vd, 0, (uint64_t)acc);                                                       \
            break;                                                                              \
        case 64:                                                                                \
            WR_V64(vd, 0, (uint64_t)acc);                                                        \
            break;                                                                              \
    }                                                                                           \
}
...
{                                                                                                       \
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {                                                    \
        ...
        switch (eew) {                                                                                  \
        case 32:                                                                                        \
            acc = glue(HELPER, _d)(env, acc, helper_fcvt_d_s(env, (float32)V32(vs2,ei), env->frm));  \
            break;                                                                                      \
        }                                                                                               \
    }                                                                                                   \
    switch (eew) {                                                                                      \
...
void glue(helper_vfslide1up, POSTFIX)(CPUState *env, uint32_t vd, int32_t vs2, float64 imm)
{
    ...
    if (env->vstart == 0
        ...
        switch (eew) {
        case 32:
            WR_V32(vd, 0, unbox_float32(env, imm));
            break;
        case 64:
            WR_V64(vd,0, imm);
            break;
