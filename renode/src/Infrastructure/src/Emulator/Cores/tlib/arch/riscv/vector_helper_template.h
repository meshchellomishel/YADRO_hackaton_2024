...
void glue(glue(helper_vlxei, BITS), POSTFIX)(CPUState *env, uint32_t vd, uint32_t rs1, uint32_t vs2, uint32_t nf)
{
    ...
    if (index_emul == RESERVED_EMUL || data_emul == RESERVED_EMUL || V_IDX_INVALID_EMUL(vd, data_emul) || V_IDX_INVALID_EMUL(vs2, index_emul) || V_INVALID_NF(vd, nf, data_emul)) {
        raise_exception_and_sync_pc(env, RISCV_EXCP_ILLEGAL_INST);
    }
...
{                                                                                                                                       \
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {                                                                                    \
        ...
        switch (eew) {                                                                                                                  \
        case 8:                                                                                                                         \
            WR_V16(vd,ei,OP(V16(vd,ei*2), (uint16_t)V08(vs2,ei), (int16_t)V08(vs1,ei)));                                                \
            break;                                                                                                                      \
        case 16:                                                                                                                        \
            WR_V32(vd,ei,OP(V32(vd,ei*4), (uint32_t)V16(vs2,ei*2), (int32_t)V16(vs1,ei*2)));                                            \
            break;                                                                                                                      \
        case 32:                                                                                                                        \
            WR_V64(vd,ei,OP(V64(vd,ei*4), (uint64_t)V32(vs2,ei*4), (int64_t)V32(vs1,ei*4)));                                            \
            break;                                                                                                                      \
        default:                                                                                                                        \
            raise_exception_and_sync_pc(env, RISCV_EXCP_ILLEGAL_INST);                                                                  \
            break;                                                                                                                      \
...
{                                                                                                       \
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {                                                    \
        ...
        switch (eew) {                                                                                  \
        case 8:                                                                                         \
            acc = OP((uint8_t)acc, V08(vs2,ei));                                            \
            break;                                                                                      \
        case 16:                                                                                        \
            acc = OP((uint16_t)acc, V16(vs2,ei*2));                                          \
            break;                                                                                      \
        case 32:                                                                                        \
            acc = OP((uint32_t)acc, V32(vs2,ei*4));                                          \
            break;                                                                                      \
        case 64:                                                                                        \
            acc = OP((uint64_t)acc, V64(vs2,ei*8));                                          \
            break;                                                                                      \
        }                                                                                               \
    }                                                                                                   \
    switch (eew) {                                                                                      \
...
{                                                                                                       \
    ...
    for (int ei = env->vstart; ei < env->vl; ++ei) {                                                    \
        ...
        switch (eew) {                                                                                  \
        case 8:                                                                                         \
            acc = OP((int8_t)acc, V08(vs2,ei));                                            \
            break;                                                                                      \
        case 16:                                                                                        \
            acc = OP((int16_t)acc, V16(vs2,ei*2));                                          \
            break;                                                                                      \
        case 32:                                                                                        \
            acc = OP((int32_t)acc, V32(vs2,ei*4));                                          \
            break;                                                                                      \
        case 64:                                                                                        \
            acc = OP((int64_t)acc, V64(vs2,ei*8));                                          \
            break;                                                                                      \
        }                                                                                               \
    }                                                                                                   \
    switch (eew) {                                                                                      \
...
void glue(helper_vwredsumu_ivv, POSTFIX)(CPUState *env, uint32_t vd, int32_t vs2, int32_t vs1)
{
    ...
    switch (eew) {
    case 8:
        WR_V16(vd, 0, (acc + V16(vs1,0)));
        break;
    case 16:
        WR_V32(vd, 0, (acc + V32(vs1,0)));
        break;
    case 32:
        WR_V64(vd, 0, (acc + V64(vs1,0)));
        break;
    default:
        raise_exception_and_sync_pc(env, RISCV_EXCP_ILLEGAL_INST);
        break;
